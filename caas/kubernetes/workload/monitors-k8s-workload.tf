...
resource "datadog_monitor" "replica_available" {
  ...
  query = <<EOQ
    ${var.replica_available_time_aggregator}(${var.replica_available_timeframe}):
      max:kubernetes_state.deployment.replicas_desired${module.filter-tags.query_alert} by {${local.available_replica_group_by}} -
      max:kubernetes_state.deployment.replicas_available${module.filter-tags.query_alert} by {${local.available_replica_group_by}}
